...
namespace SamplePlugins
{
	...
	public class CustomAPIConfig : ICustomAPIConfig
	{
		...
		public CustomAPIConfig(string name)
		{
			...
			_UniqueName = name;
			_IsFunction = false;
			_EnabledForWorkflow = 0;
			_AllowedCustomProcessingStepType = 0; // None
			_BindingType = 0; // Global
			_BoundEntityLogicalName = "";
		...
		{
			_AllowedCustomProcessingStepType = (int)type;
			return this;
		}

		public CustomAPIConfig Bind<T>(BindingType bindingType) where T : Entity
		{
			_BindingType = (int)bindingType;
			_BoundEntityLogicalName = Activator.CreateInstance<T>().LogicalName;
			return this;
		}
		...
		{
			respProperty.SetNameFromAPI(_Name);
		...
		public class CustomAPIRequestParameter
		{
				...
				string name, RequestParameterType type, bool isCustomizable = false, bool isOptional = false, string logicalEntityName = null)
			{
				...
				_LogicalEntityName = logicalEntityName;
				_Type = type;
			}
